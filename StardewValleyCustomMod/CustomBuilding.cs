...
{/*
    ...
    public class CustomBuilding
    {
        ...
        public CustomBuilding(string buildingType, string nameOfIndoors, int tileX, int tileY, int tilesWide, int tilesTall, Point humanDoor, Point animalDoor, GameLocation indoors, Texture2D texture, bool magical, long owner)
        {
            this.tileX = tileX;
            this.tileY = tileY;
        ...
        public CustomBuilding(CustomBuildingBlueprint blueprint, Vector2 tileLocation)
        {
            ...
            else
            {
                if (this.daysUntilUpgrade > 0 && !Utility.isFestivalDay(dayOfMonth, Game1.currentSeason))
                {
                    ...
                    if (this.daysUntilUpgrade <= 0)
                    {
                        ...
                        this.indoors.map = Game1.content.Load<Map>("Maps\\" + bluePrint.mapToWarpTo);
                        this.indoors.name = bluePrint.mapToWarpTo;
                        this.buildingType = bluePrint.name;
                        this.texture = bluePrint.texture;
                        if (Type.op_Equality(this.indoors.GetType(), typeof(AnimalHouse)))
                        {
        ...
        protected virtual GameLocation getIndoors()
        {
            if (this.buildingType.Equals("Slime Hutch"))
            {
                ...
                GameLocation gameLocation = (GameLocation)new SlimeHutch(Game1.content.Load<Map>("Maps\\" + this.nameOfIndoorsWithoutUnique), this.buildingType);
            ...
            if (this.buildingType.Equals("Shed"))
            {
                ...
                GameLocation gameLocation = (GameLocation)new Shed(Game1.content.Load<Map>("Maps\\" + this.nameOfIndoorsWithoutUnique), this.buildingType);
                gameLocation.IsFarm = true;
                gameLocation.isStructure = true;
                foreach (Warp warp in gameLocation.warps)
                {
            ...
            GameLocation gameLocation1 = new GameLocation(Game1.content.Load<Map>("Maps\\" + this.nameOfIndoorsWithoutUnique), this.buildingType);
        ...
        public virtual void drawShadow(SpriteBatch b, int localX = -1, int localY = -1)
        {
            ...
            b.Draw(Game1.mouseCursors, position, new Rectangle?(CustomBuilding.leftShadow), Color.White * (localX == -1 ? this.alpha : 1f), 0.0f, Vector2.Zero, (float)Game1.pixelZoom, SpriteEffects.None, 1E-05f);
            for (int index = 1; index < this.tilesWide - 1; ++index)
                b.Draw(Game1.mouseCursors, position + new Vector2((float)(index * Game1.tileSize), 0.0f), new Rectangle?(CustomBuilding.middleShadow), Color.White * (localX == -1 ? this.alpha : 1f), 0.0f, Vector2.Zero, (float)Game1.pixelZoom, SpriteEffects.None, 1E-05f);
            b.Draw(Game1.mouseCursors, position + new Vector2((float)((this.tilesWide - 1) * Game1.tileSize), 0.0f), new Rectangle?(CustomBuilding.rightShadow), Color.White * (localX == -1 ? this.alpha : 1f), 0.0f, Vector2.Zero, (float)Game1.pixelZoom, SpriteEffects.None, 1E-05f);
        }
