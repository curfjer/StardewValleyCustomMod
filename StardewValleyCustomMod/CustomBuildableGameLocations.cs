...
namespace CustomFarmBuildings.Locations
{
    ...
    {
        /*public List<CustomBuilding> buildings = new List<CustomBuilding>();
        ...
        public CustomBuildableGameLocation()
        {
        }

        public CustomBuildableGameLocation(Map m, string name)
          : base(m, name)
        {
        }
        ...
        public override void DayUpdate(int dayOfMonth)
        {
            ...
            foreach (CustomBuilding building in this.buildings)
        ...
        public virtual void timeUpdate(int timeElapsed)
        {
            foreach (CustomBuilding building in this.buildings)
            {
                if (building.indoors != null && Type.op_Equality(building.indoors.GetType(), typeof(AnimalHouse)))
                {
        ...
        public CustomBuilding getBuildingAt(Vector2 tile)
        {
        ...
        public bool destroyStructure(CustomBuilding b)
        {
        ...
        public override bool isCollidingPosition(Microsoft.Xna.Framework.Rectangle position, xTile.Dimensions.Rectangle viewport, bool isFarmer, int damagesFarmer, bool glider, Character character, bool pathfinding, bool projectile = false, bool ignoreCharacterRequirement = false)
        {
            if (!glider)
            {
                foreach (CustomBuilding building in this.buildings)
                {
                    ...
                    {
                        if (character != null && Type.op_Equality(character.GetType(), typeof(FarmAnimal)))
                        {
        ...
        public void tryToUpgrade(CustomBuilding toUpgrade, CustomBuildingBlueprint blueprint)
        {
            if (toUpgrade != null && blueprint.name != null && toUpgrade.buildingType.Equals(blueprint.nameOfBuildingToUpgrade))
            {
                ...
                else
                {
                    ...
                    if (Type.op_Equality(toUpgrade.indoors.GetType(), typeof(AnimalHouse)))
        ...
        public CustomBuilding getBuildingUnderConstruction()
        {
            foreach (CustomBuilding building in this.buildings)
            {
                if (building.daysOfConstructionLeft > 0 || building.daysUntilUpgrade > 0)
        ...
        public bool buildStructure(CustomBuilding b, Vector2 tileLocation, bool serverMessage, Farmer who)
        {
        ...
        public bool buildStructure(BluePrint structureForPlacement, Vector2 tileLocation, bool serverMessage, Farmer who, bool magicalConstruction = false)
        {
            ...
            if (stringHash <= 1972213674U)
            {
                if (stringHash <= 846075854U)
                {
                    ...
                    {
                        building = (CustomBuilding)new Stable(structureForPlacement, tileLocation);
                        goto label_40;
                    }
                    else
                        goto label_39;
            ...
            else if (stringHash <= 2601855023U)
            {
                ...
                {
                    building = (CustomBuilding)new JunimoHut(structureForPlacement, tileLocation);
                    goto label_40;
                }
                else
                    goto label_39;
            ...
            else if ((int)stringHash != -1111878468)
            {
                if ((int)stringHash != -560689829)
                {
                    ...
                    {
                        building = (CustomBuilding)new Mill(structureForPlacement, tileLocation);
                        goto label_40;
                    }
                    else
                        goto label_39;
                ...
                goto label_39;
            building = (CustomBuilding)new Coop(structureForPlacement, tileLocation);
            goto label_40;
        label_36:
            building = (CustomBuilding)new Barn(structureForPlacement, tileLocation);
            goto label_40;
        label_39:
            building = new CustomBuilding(structureForPlacement, tileLocation);
        label_40:
